...
import { CreateUser, User, UserFromServer } from './types';
...
window.onload = () => {
	...
	callApi<UserFromServer>('https://js-course-2.firebaseio.com/users.json').then(responseBody => {
	...
	function renderUsers(): void {
		...
		const usersFragment: DocumentFragment = userList
			.map(user => {
		...
		function removeOnClick(listItem: HTMLLIElement) {
			listItem.addEventListener('click', () => {
				const userIdToRemove = listItem.getAttribute('DATA-ID');

				callApi<void>(`https://js-course-2.firebaseio.com/users/${userIdToRemove}.json`, 'DELETE').then(() => {
	...
	const button = document.getElementsByClassName('add-button')[0] as HTMLButtonElement;

	button.addEventListener('click', () => {
		const newNameInput = document.getElementsByClassName('name-input')[0] as HTMLInputElement;
		...
		if (newName !== '') {
			...
			if (userAlreadyExists) {
				...
				callApi<CreateUser>('https://js-course-2.firebaseio.com/users.json', 'POST', newUser).then(responseBody => {
